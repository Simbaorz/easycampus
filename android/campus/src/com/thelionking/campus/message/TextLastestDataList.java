 ...
 */
public class TextLastestDataList extends DataList {

    public TextLastestDataList(String name, Context context, String tabName) {
    ...
    public List<Joke> getJokeFromServer(final String action) {
        ...
        try {
            URL serverUrl = null;
            if(fromTop) {
                serverUrl = new URL(CommonUtil.spliceUrl(action, topStart, Joke.TEXT, Joke.LASTEST_MODE, 0));
                Log.d(TAG, "get from url : " + CommonUtil.spliceUrl(action, topStart, Joke.TEXT, Joke.LASTEST_MODE, 0));
            }else{
                serverUrl = new URL(CommonUtil.spliceUrl(action, bottomStart, Joke.TEXT, Joke.LASTEST_MODE, 0));
                Log.d(TAG, "get from url : " + CommonUtil.spliceUrl(action, topStart, Joke.TEXT, Joke.LASTEST_MODE, 0));
            }
