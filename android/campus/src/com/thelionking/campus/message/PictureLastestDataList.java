 ...
 */
public class PictureLastestDataList extends DataList {

    public PictureLastestDataList(String name, Context context, String tabName) {
        super(name, context, tabName);
    }
    ...
    public List<Joke> getJokeFromServer(final String action) {
        ...
        try {
            URL serverUrl = null;
            if(fromTop) {
                serverUrl = new URL(CommonUtil.spliceUrl(action, topStart, Joke.PICTURE, Joke.LASTEST_MODE, 0));
                Log.d(TAG, "get from url : " + CommonUtil.spliceUrl(action, topStart, Joke.PICTURE, Joke.LASTEST_MODE, 0));
            }else{
                serverUrl = new URL(CommonUtil.spliceUrl(action, bottomStart, Joke.PICTURE, Joke.LASTEST_MODE, 0));
                Log.d(TAG, "get from url : " + CommonUtil.spliceUrl(action, topStart, Joke.PICTURE, Joke.LASTEST_MODE, 0));
            }
